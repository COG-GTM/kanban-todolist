...
> **Heads up — parallel workstreams.** This phase can be built at the same time as its sibling phases (Phase 5 — Edit Modal and Phase 6 — Search/Filter/Sort). Each runs as its own workstream/branch and edits overlapping files (`render.js`, `events.js`, `index.html`, `css/styles.css`) **additively**. After *every* parallel workstream merges, **re-open and refresh `index.html` and click through all of the combined features together** to make sure nothing was overwritten or broken before you move on.
